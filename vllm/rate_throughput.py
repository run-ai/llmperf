...
async def send_requests_periodically(prompt, max_tokens, interval_seconds, remaining_requests):
    global start_time
    tasks = []
...
async def main():
    ...
    start_time = timer()
    send_task = asyncio.create_task(send_requests_periodically(prompt, args.max_tokens, requests_interval, remaining_requests))
